...
Episode.collection.filter(guid="fe5ae151-2db3-4038-b0e0-4e3c678a7064")
    Args:
    ...
    for item in feed.entries:
        if not Episode.collection.filter("guid","==",item.guid).get():
            episode = Episode()
            episode.title=item.title
            episode.description=item.description
            episode.pub_date=parser.parse(item.published)
            episode.link=item.link
            episode.image=podcast_image
            episode.podcast_name=podcast_title
